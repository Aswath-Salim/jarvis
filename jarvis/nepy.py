...
def get_gemini_response(prompt: str) -> str:
    ...
    persona_prompt = "Act as a kind and caring caretaker for an elderly person. Speak in short, gentle sentences. Use warm and supportive language. If the user seems sad or tired, offer emotional support like a friend or nurse would. Your response should be less than 50 words."
    ...
    if "time" in command:
        ...
        if name:
            ...
            if set_user_name(user_id, name):
                speak(f"Okay, I'll remember your name is {name}.")
            else:
                speak("Sorry, I couldn't store your name.")
        else:
        ...
        if name:
            ...
            speak("I don't know your name yet. You can tell me by saying 'My name is <your name>'.")
        ...
        if len(location_parts) >= 2:
            ...
            if db_utils.store_user_location(user_id, city, state):
                speak(f"Okay, I'll remember you live in {city}, {state}.")
            else:
                speak("Sorry, I couldn't store your location.")
        else:
            speak("Please tell me your city and state. For example, 'I live in London England'.")
        ...
        if city_match:
            ...
            if weather_data:
                ...
                speak(f"The temperature in {city} is {temp}°C with {cond} (from cache).")
            else:
                ...
                if weather_key is None:
                    speak("Weather API key not found in .env file.")
                    return
            ...
            user_id = os.environ.get("DEFAULT_USER_ID", "default_user")  # Use environment variable for user ID, default to "default_user"
            user_location = db_utils.get_user_location(user_id)
            if user_location:
                ...
                if weather_data:
                    temp = weather_data["temperature"]
                    cond = weather_data["condition"]
                    speak(f"The temperature in {city}, {state} is {temp}°C with {cond} (from cache).")
                else:
                    weather_key = os.getenv("WEATHER_API_KEY")
                    if weather_key is None:
                        speak("Weather API key not found in .env file.")
                    ...
                    try:
                        ...
                        weather_data = {"temperature": temp, "condition": cond}
                        if db_utils.store_weather_data(city, weather_data):
                            speak(f"The temperature in {city}, {state} is {temp}°C with {cond}.")
                        else:
                            speak(f"The temperature in {city}, {state} is {temp}°C with {cond}, but I couldn't save it to the cache.")
                    except requests.exceptions.RequestException as e:
                        speak(f"Could not fetch weather for {city}. Error: {e}")
                    except KeyError:
                        speak(f"Could not find weather information for {city}. Please ensure the city name is correct.")
            else:
                speak("I don't have your location. Please tell me where you are, like 'I live in London England'.")
